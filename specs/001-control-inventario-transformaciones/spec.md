...
- Q: ¿Cómo debe actuar el sistema si un barman recibe mercadería en barra de un proveedor que aún no fue registrado previamente por el Administrador? → A: Opción A (+ Proveedor no listado): El barman selecciona "+ Proveedor no listado" e ingresa el nombre comercial rápido para no frenar la recepción en barra; el sistema lo asienta inmediatamente en la base de datos y lo deja activo para que el Administrador revise o complete sus datos fiscales en su panel.
- Q: ¿Cómo debe presentar la pantalla de Corte de Cierre el conteo inicial y el cálculo de salidas mientras el barman digita su inventario final? → A: Opción A (Pill informativo y cálculo en tiempo real): Cada tarjeta de producto en el Corte de Cierre muestra debajo de su nombre el cintillo: [Inició: X.XX | Ingresos: +Y.YY | Disp: Z.ZZ] y, a medida que el barman digita su conteo final, calcula de manera dinámica la salida o venta estimada (Venta/Consumo estimada: Disp - Conteo Actual), brindando certeza operativa instantánea en barra sin bloquear la entrega de turno.
- Q: ¿Cómo debe presentarse el balance en el informe operativo en PDF cuando el Administrador consulte un turno que aún se encuentra abierto en servicio activo? → A: Opción A (Turno en Vivo con Stock Teórico en Custodia): El PDF de auditoría rotula el estado 'TURNO EN VIVO (EN SERVICIO)' y calcula el Stock Teórico que debe existir físicamente en barra en ese momento exacto (Apertura + Ingresos + Traspasos Entrantes - Traspasos Salientes - Bajas - Rellenos Consumidos + Rellenos Producidos), dejando la aclaración de 'Corte Final Pendiente de Cierre' para inspecciones sorpresa del Administrador.
...
- **FR-033**: Al completar el Corte de Cierre de Turno (`CorteInventarioScreen`), la aplicación DEBE generar de manera obligatoria el "Acta Oficial de Cierre de Turno y Balance de Inventario" en PDF vectorial presentando los nombres oficiales y descriptivos de los productos (sin nombres genéricos), eliminando la columna redundante de "Tipo" y estructurando la tabla oficial de reconciliación requerida por el reglamento: `['#', 'PRODUCTO', 'INICIAL', 'INGRESOS (+)', 'RELLENOS (±)', 'BAJAS (-)', 'TOTAL CIERRE']` con resumen al pie de totales, desplegando el diálogo con el botón destacado "COMPARTIR EN WHATSAPP".
...
- **FR-038**: En el formulario de Recepción de Mercadería (`ingreso_mercaderia_screen.dart`), el sistema DEBE desplegar un selector con búsqueda rápida de proveedores activos registrados en la base de datos y permitir la opción "+ Proveedor no listado" para ingresar el nombre de un proveedor imprevisto sin frenar la recepción en barra, registrándolo automáticamente en el catálogo para regularización del Administrador.
...
- **FR-040**: El sistema DEBE compilar y generar un Informe Operativo Integral en formato PDF (`informe_operativo_turno_pdf_service.dart`) para cualquier turno seleccionado por el Administrador. Si el turno está abierto, DEBE rotularlo como 'TURNO EN VIVO (EN SERVICIO)' calculando el saldo teórico en custodia en tiempo real para inspecciones sorpresa; si el turno ya cerró, DEBE detallar el balance físico final definitivo, discrepancias y la liquidación económica del barman (sueldo base + comisiones brutas = total a pagar).
...
### User Story 22 - Balance Integral en Conteo de Apertura PDF, Reconciliación Completa en PDF de Cierre y Visualización de Conteo Inicial en Pantalla de Cierre (Priority: P1)

Como Barman y Supervisor de Punto Frío, quiero que el PDF de Conteo Inicial y Balance en Turno refleje fielmente las compras y recepciones de mercadería realizadas durante la jornada sumándolas al disponible, que el PDF de Cierre de Turno muestre los nombres reales de los productos y la tabla completa de balance (Conteo Inicial, Ingresos, Rellenos, Bajas y Total Cierre dejando el turno) sin la columna redundante de tipo, y que en la pantalla móvil de Corte de Cierre se me indique visualmente con cuánto inicié cada producto y cuánto ingresó, para contar con total certeza y transparencia en la entrega de la custodia de la barra.

**Independent Test**:
1. Apertura y Recepción: Iniciar turno con conteo inicial (ej. 36 Moema, 12 Huari), registrar una recepción/compra de mercadería (+12 Moema) y pulsar "VER / RE-IMPRIMIR CONTEO DE APERTURA": el PDF debe mostrar en la columna `INGRESOS (+)` el valor `+12.00` y en `TOTAL DISP.` `48.00` sin mostrar guiones (`-`).
2. Pantalla de Corte de Cierre: Al ingresar a realizar el corte final, cada tarjeta de producto debe mostrar claramente debajo de su nombre: `Inició: 36.00 | Ingresos: +12.00 | Disp: 48.00`, permitiendo al barman saber su saldo de referencia.
3. PDF de Cierre de Turno: Confirmar el cierre de turno: el PDF generado debe mostrar los nombres oficiales de los productos (ej. "moema lata", "huari"), eliminar la columna "Tipo", y estructurar las columnas de balance: `[#] | [PRODUCTO] | [INICIAL] | [INGRESOS (+)] | [RELLENOS (±)] | [BAJAS (-)] | [TOTAL CIERRE]`.
